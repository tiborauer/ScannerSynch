...
%   Methods
%       obj = TCPClass(port, [switches])    Constructor
...
%       dat = ReceiveData(n,dtype,[swiches])   Read a certain number of typed data
...
classdef TCPClass < handle
    ...
    methods
        function obj = TCPClass(port,varargin)
            if nargin > 1
        ...
        function Connect(obj,address)
            obj.Log(sprintf('Connecting to host at %s...',address))
            obj.Connection = pnet('tcpconnect',address,obj.Port);
            obj.Log('Connection established')
        end
        
        function WaitForConnection(obj)
            obj.Log('Waiting for connecting to client...');
...
%             obj.TimeOut = 0.1;
        ...
        function SendData(obj,dat,varargin)
            pnet(obj.Connection,'write',dat,varargin{:});
        end
        
        function dat = ReceiveData(obj,n,dtype,varargin)
            dat = [];
        ...
        function ReadBytesToBuffer(obj)
            while obj.BytesAvailable
                obj.Buffer = [obj.Buffer obj.ReceiveData(obj.BytesAvailable,'uint8')];
            end
        ...
        function Flush(obj)
            while obj.BytesAvailable
                obj.ReceiveData(obj.BytesAvailable,'uint8');
            end
        end
